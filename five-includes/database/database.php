 ...
 * @since		1.0
 */
class FiveDatabase extends Object
{
	/**
	...
	*/
	 ...
	 * @return FiveDatabase A database object
	 * @since 1.5
	*/
	function &getInstance( $options	= array() )
	{
		...
		if (empty($instances[$signature]))
		{
			...
			if (file_exists($path)) {
				...
				trigger_error( 'Unable to load Database Driver:'.$driver, E_USER_ERROR );
				return $error;
			}

			$adapter	= 'FiveDatabase'.$driver;
			$instance	= new $adapter($options);

			if ( $error = $instance->getErrorMsg() )
			{
				$error = 'Unable to connect to the database:'.$database;
	...
	/**
	 * Get the sql tree, in part or in whole
	 * 
	 * @param $key
	 * @return array
	 */
	function getTree($key=null)
	{
		if (!is_null($key))
			return $this->_sql_tree[$key];
		else
		{
			$query = '';
			foreach ((array)$this->_sql_tree as $type => $items)
			{
				switch ($type)
				{
					default: $sep = ' '; break;
					...
					case 'GROUP BY':  
						$sep = ', '; break;
					case 'WHERE':  $sep = ' AND '; break;
				}
				
				if (!empty($items))
					$query .= " $type " . implode($sep, $items);
			}
			return $query;
		}
	}
	...
	/**
	 * Adds values to the sql tree
	 * 
	 * @param unknown_type $key
	 * @param unknown_type $value
	 * @return boolean 
	 */
	function setTree($key, $value=null)
	{
		if (!is_null($value))
		{
			if (is_array($value))
				$this->_sql_tree[$key] = $value;
			else
				$this->_sql_tree[$key][] = $value;
	...
	function resetTree()
	{
		foreach ((array)$this->_sql_tree as $key => $value)
			$this->_sql_tree[$key] = array();
	}
	
	/**
	 * Sets the well formed query
	 * 
	 * @return string 
	 */
	function queryTree()
	{
		$this->setQuery( $query = $this->getTree() );
		$this->resetTree();
		return $query;
	}
	...
	*/
	...
	function Execute( $query )
	{
		...
		if (preg_match('#^select#i', $query )) {
			...
			return new FiveRecordSet( $result );
		} else {
			$result = $this->query();
			if ($result === false) {
				return false;
			} else {
				return new FiveRecordSet( array() );
			}
		}
	}
