...
/**
 * Initializing the Permalink structure
 * 
 * This will be considered the default structure that is received by the system last.
 */
Router::connect('/index', array('controller' => 'home', 'action' => ''));
 ...
 */
	function connect($route, $defaults = array(), $options = array()) {
		...
		if ($routeClass !== 'FiveRoute' && !is_subclass_of($Route, 'FiveRoute')) {
			trigger_error(__('Route classes must extend FiveRoute', true), E_USER_WARNING);
			return false;
		}
		$self->routes[] =& $Route;
		return $self->routes;
		...
		if (is_array($url)) {
			...
			if (empty($url)) {
				...
				if (!empty($params['plugin']) && $params['plugin'] !== $params['controller']) {
					$output .= $params['plugin'] . '/';
				}
				$output .= $params['controller'] . '/' . $url;
			}
 ...
 *
 * @return FiveRoute Matching route object.
 * @access public
 * @static
 */
 ...
 * @access public
 */
	function FiveRoute($template, $defaults = array(), $options = array()) {
 ...
 * @package cake.libs
 */
class PluginShortRoute extends FiveRoute {
